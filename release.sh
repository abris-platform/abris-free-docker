...
for i in ${!PROJECT_ID[@]}; do
	if [[ "${PROJECT_ID[$i]}" != "68" ]]; then
		if [[ "${PROJECT_ID[$i]}" != "4" ]]; then
			JOB[$i]=$(curl --header "PRIVATE-TOKEN: ${CI_API_TOKEN}" "https://gitlab.abris.site/api/v4/projects/${PROJECT_ID[$i]}/jobs?scope=success" | jq '.[0]'); # JSON последнего успешного билда проекта.
		else
			JOB[$i]=$(curl --header "PRIVATE-TOKEN: ${CI_API_TOKEN}" "https://gitlab.abris.site/api/v4/projects/${PROJECT_ID[$i]}/jobs?scope=success" | jq 'map (select (.name=="build-free")) | .[0]'); # JSON последнего успешного билда сервера (больше одного job'а в конвеере).
		fi
		COMMIT[$i]=$(echo ${JOB[$i]} | jq --raw-output '.commit.short_id');
		JOB[$i]=$(echo ${JOB[$i]} | jq '.id'); # Замена JSON'а на id процесса.
		DESCRIPTION="${DESCRIPTION}* **${PROJECT_NAME[$i]}** [artifact](https://gitlab.abris.site/abris-lab/${PROJECT_NAME[$i]}/-/jobs/${JOB[$i]}/artifacts/download) (job [${JOB[$i]}](https://gitlab.abris.site/abris-lab/${PROJECT_NAME[$i]}/-/jobs/${JOB[$i]})), commit: https://gitlab.abris.site/abris-lab/${PROJECT_NAME[$i]}/commit/${COMMIT[$i]}\n"; # Добавление ссылки в описание.
	else
		JOB[$i]=$(curl --header "PRIVATE-TOKEN: ${CI_API_TOKEN}" "https://gitlab.abris.site/api/v4/projects/${PROJECT_ID[$i]}/jobs?scope=success" | jq 'map (select (.name=="repack")) | .[0]'); # JSON последнего успешного деплоя до релиза.
		COMMIT[$i]=$(echo ${JOB[$i]} | jq --raw-output '.commit.short_id');
		JOB[$i]=$(echo ${JOB[$i]} | jq '.id'); # Замена JSON'а на id процесса.
	fi
